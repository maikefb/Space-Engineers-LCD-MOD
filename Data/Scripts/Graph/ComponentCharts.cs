...
{
    [MyTextSurfaceScript("ComponentCharts", "Componentes")]
    public class ComponentCharts : MyTextSurfaceScriptBase
    {
        ...
        private static readonly Regex RxSection = new Regex(@"\[ComponentCharts\](.*?)(?:\r?\n\r?\n|\Z)", RegexOptions.Singleline | RegexOptions.IgnoreCase);
        ...
        public override void Run()
        {
            using (var frame = Surface.DrawFrame())
            {
                ...
                // cabeçalho + linha em branco
                var pos = INFO_POS;
                sprites.Add(Text("Estoque de componentes", pos, 0.85f));
        ...
        private int GetMaxRowsFromSurface(float listStartY)
        {
            ...
            float available = Math.Max(0f, surfH - listStartY - 10f); // margem inferior
        ...
        private List<KeyValuePair<string,double>> ReadItems(IMyTerminalBlock lcd)
        {
            ...
            var sec = RxSection.Match(data);
            if (!sec.Success) return list;
        ...
        {
            if (v >= 1000) return Math.Round(v).ToString("#,0", Pt);
